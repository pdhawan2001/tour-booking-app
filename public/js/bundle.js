...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaW5kb2xlbnR0cm91dCIsImEiOiJja3dmYjY1dncwMHc0Mm5tbHBweXQ5aGRoIn0.AFnMsS7IRziusCYl-5pTiA';
  var map = new mapboxgl.Map({
    container: 'map',
    // container ID // that's why we created map file in pug with 'map' id
...
var showAlert = function showAlert(type, msg) {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // inside of body but right at beginning
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            (0, _alert.showAlert)('error', _context.t0.response.data.message); // this comes from axios documentation
    ...
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            res = _context2.sent;
            if (res.data.status = 'success') location.reload(true); // true helps us to reload the server too
...
var stripe = Stripe('pk_test_51K6DPRSGafvRGKQ4132to9vhgiWPGGLsLNIV5gbhTtCmIlSBM47UDUKMzG16IlTFN0eSGRiO0ogFFV7bDImJ9UKL00lFbSf9l6');
...
if (mapBox) {
  var locations = JSON.parse(mapBox.dataset.locations); // map is the css id we specified, whatever we put in data attribute lik data-locations(in tour.pug file) and will be called dataset.locations because it is uses as data/locations or data-locations
...
if (loginForm) {
  loginForm.addEventListener('submit', function (e) {
    ...
    var password = document.getElementById('password').value; // it gets the email and password value

    (0, _login.login)(email, password);
  });
}

if (logOutBtn) logOutBtn.addEventListener('click', _login.logout); // whenever the log Out button is clicked we want to listen to it and then logout the user
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    ...
    form.append('photo', document.getElementById('photo').files[0]); // files are an array
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    ...
    var tourId = e.target.dataset.tourId; // data-tour-id, this id automatically gets converted into 'tourId' in JS // element.target is basically the button which was clicked, that is which triggered tthis event listner to be fired
